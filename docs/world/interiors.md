...
| **WALL-HUNG** | `banner_warm`/`banner_ice` 1×2 · `kite_hung` 3×2 | Pure front elevation hung ON the wall — no floor contact line. Placed via `wall_mount(..., solid=False)`: banners at the default `face_row=1` (the `lamp_rack` pattern), `kite_hung` at `face_row=0` so it hangs over the cap band, clear of a 3×3 altar beneath it. The MIN-1 lesson: two Lumenaries must not share one dressing — re-skin each hall to its warden's element with these + the region's own outdoor objects (e.g. `pale_vault_brazier`/`pale_vault_ice_spire` via a raw object entry). |
