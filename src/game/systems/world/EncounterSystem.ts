 ...
 * a Lantern Gift the player lacks) gets a per-step chance to fire; on a hit we pick
 * a kin from the zone's weighted table and a level in its range, and emit a typed
 * intent the scene turns into a battle. Zone rects are authored to match the painted
 * terrain (see docs/world/level-design.md).
 */
...
export class EncounterSystem {
  ...
  roll(tx: number, ty: number, abilities: ReadonlySet<AbilityId>): EncounterIntent | null {
